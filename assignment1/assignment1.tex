...
There are three different colours, 9 balls, 2  balls are picked from the urn uniformly at random. The size of the sample space is $|\Omega| = \texttt{colours}^{\texttt{picks}} = 3^2 = 9$, the possible outcomes are:
$$ \Omega = \left\lbrace \texttt{RR, RO, OO, OR, OB, BB, BR, BO} \right\rbrace $$
...
Before picking any ball the probability of picking an orange ball is $ 1 \times 3 / 9 + 2 \times 3 = 1 $ Thus the expected number of orange balls picked is 1.
...
\begin{align}
\mathbf{E}[XY] &= \sum\limits{x,y} Pr(X=x,Y=y) \\
&= \sum\limits{x}\sum\limits{y} xy Pr(X=x,Y=y) \\
&= \sum\limits{x}\sum\limits{y} xy Pr(X=x)Pr(Y=y) \texttt{  (Independent)} \\
&= \sum\limits{x}x Pr(X=x)\sum\limits{y} y Pr (Y=y) \\
&= \mathbf{E}[X] \mathbf{E}[Y] \\
\end{align}
where step step 8 follows from X and Y being independent random variable.
\subsection{Question 3}
\subsection{Question 4}
\subsection{Question 5}
