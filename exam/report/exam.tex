...
\end{align*}
The lowest error was measured when b was 2 or $e$. The above parameters was obtained during training, below is the error of applying the SVM, with the above parameters on the training and test data.
...
\begin{equation}
X_{f_{a,b}} = \dfrac{1}{\sigma_\mathbf{X}} + \dfrac{- \mu_\mathbf{X}}{\sigma_\mathbf{X}} = \dfrac{\mathbf{X} - \mu_\mathbf{X} }{\sigma_\mathbf{X}}
\label{norm}
\end{equation}
...
x_d = (\theta -b_j)/a
\end{equation}
Thus the parameters of the affine transformation, uniquely defines the threshold, and therefore have no influence on the data.
...
First we note that the VC-dimension of a hyperplane in the dimension $d$ is $d+1$, that is we can shatter at most $d+1$ points. \\
First we consider a set on $N=d+1$ points in dimension $d$. We want to show that it is possible for a hyperplane in dimension $d$ to shatter these points. If we start by constructing a $d+1 \times d+1$ matrix for the data set. The dimension of each data point is $d$, and we include $x_0 =1$ so each datapoint is a $d+1$ vector.
We set the rest of the matrix to be the identity matrix and construct a vector y of $d+1$ targetvalues, each target being $\pm1$. We want to find a weight vector w satisfying:
$$
\text{sign}(Xw)=y
$$
however we note that following the definition of X allows to calculate this by simply taking the value, instead of the sign. We can then determine w such that we can shatter the points
$$
w = X^{-1}y
$$
meaning we can shatter $d+1$ points. \\
To show that it is not possible for the hyperplane to shatter $d+2$ points, we note that we have more vectors than dimensions, therefore the points must be linearly dependent and we therefore cannot define a w which shattered all the points. \\
...
\end{equation}
The minus one is because we don't count 1 as a monomial.
...
\begin{equation}
d_{vc} = \left( \binom{d+Q}{Q}-1 \right) + 1
\end{equation}
...
\end{equation}
which clearly holds for all $Q,d \geq 0$
\subsection{Question 2}
From the lecture notes we have the following VC generalization bound.
\begin{equation}
